...
async function init(){buildMaintenanceMenu();bindUI();const [schools,alcaldias,agebs,subs,fracs,mant,ref,famPot,p123_2026,alcApoyo]=await Promise.all([loadSchools(),fetchJsonSafe(DATA_PATHS.alcaldias),fetchJsonSafe(DATA_PATHS.agebs),fetchJsonSafe(DATA_PATHS.subsidencias),fetchJsonSafe(DATA_PATHS.fracturamiento),fetchJsonSafe(DATA_PATHS.mantenimiento),fetchJsonSafe(DATA_PATHS.reforzamiento),fetchJsonSafe(DATA_PATHS.famPotenciado),fetchJsonSafe(DATA_PATHS.programa123_2026),fetchJsonSafe(DATA_PATHS.alcaldiaApoyo)]);allSchools=schools;joinImprovements(allSchools,mant||[],ref||[],famPot||[],p123_2026||[],alcApoyo||[]);filteredSchools=[...allSchools];alcaldiasGeoJSON=alcaldias;agebsGeoJSON=agebs;subsidenciasGeoJSON=subs;fracturamientoGeoJSON=fracs;drawBoundaries();drawExtraLayers();populateFilters();restoreState();updateMap();}
...
function colorForSchool(s){if(activeMode==='obs_fractura')return s.reforzamiento?OBS_COLORS.reforzada:OBS_COLORS.fractura;if(activeMode==='obs_subsidencia')return s.reforzamiento?OBS_COLORS.reforzada:OBS_COLORS.subsidencia;if(activeMode==='obs_combinada')return s.reforzamiento?OBS_COLORS.reforzada:OBS_COLORS.combinada;if(activeMode==='fam_regular')return '#0f766e';if(activeMode==='programa_123')return '#2563eb';if(activeMode==='fam_potenciado')return '#ca8a04';if(activeMode==='fam_reforzamiento')return '#7c3aed';if(activeMode==='programa_123_2026')return '#0891b2';if(activeMode==='alcaldia_apoyo')return '#be123c';if(activeMode==='ambas')return '#111827';return COLORS[s.clasificacion]}
function borderForSchool(s){if(s.mantenimiento&&s.reforzamiento)return '#111827';if(s.reforzamiento)return '#7c3aed';if(s.alcaldiaApoyo)return '#be123c';if(s.programa123_2026)return '#0891b2';if(s.famPotenciado)return '#ca8a04';if(s.mantenimiento)return isILIFE(s.mantenimiento)?'#0f766e':'#2563eb';return '#fff'}
function buildPopup(s){const support=matchingSupportFields(s);return `<div class="popup-title">${escapeHtml(s.nombre)}</div><div class="popup-meta">CCT: ${escapeHtml(s.ccts.join(', ')||'No registrado')}<br>Alcaldía: ${escapeHtml(s.alcaldia||'No registrada')}<br>${activeMetricLine(s)}</div><div class="popup-flags">${s.mantenimiento&&isILIFE(s.mantenimiento)?'<span class="mini-tag teal">FAM Regular 2025</span>':''}${s.mantenimiento&&isDGCOP(s.mantenimiento)?'<span class="mini-tag blue">1, 2, 3 por mi Escuela</span>':''}${s.famPotenciado?'<span class="mini-tag gold">FAM Potenciado 2025</span>':''}${s.reforzamiento?'<span class="mini-tag purple">FAM Reforzamiento estructural</span>':''}${s.programa123_2026?'<span class="mini-tag blue">1, 2, 3 por mi Escuela 2026</span>':''}${s.alcaldiaApoyo?'<span class="mini-tag warning">Alcaldía</span>':''}${support.length?'<span class="mini-tag warning">✓ Apoyo previo</span>':''}</div>`}
...
function hasAnySupport(s){return !!(s.mantenimiento||s.famPotenciado||s.reforzamiento||s.programa123_2026||s.alcaldiaApoyo)}
